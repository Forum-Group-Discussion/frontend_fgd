...
export default function FullThread() {
  ...
  const [date, setDate] = useState("2022-07-08T15:51:26.533853");
  const [convert, setConvert] = useState(null);
  const [timestamp, setTimestamp] = useState(null);
  const fetchData = useCallback(() => {
    const response = axiosInstance.get("v1/thread/" + params.id).then((response) => {
      setDetailThread(response.data.data);
            ...
            <div id="content" className="lg:grid grid-cols-2 h-[80vh] md:h-[80vh] xl:h-[90vh]">
              ...
              <div className="">
                <div id="header" className="lg:absolute top-0 right-0 left-[50%] px-4 py-3 font-semibold text-xs md:text-sm text-slate-900 bg-slate-50/90 ring-1 ring-slate-900/10">
                  ...
                  <IoIosClose id="close-button" onClick={handleCloseFull} size={40} className="absolute right-1 top-1 cursor-pointer" />
                </div>
                <div id="content-box" className="lg:mt-12 lg:mb-32">
                  <div id="content" className="scrollbar text-xs md:text-sm p-4 overflow-auto h-[57vh] md:h-[50vh] lg:h-[50vh] xl:h-[55vh] 2xl:h-[60vh]">
                    <div id="identity" onClick={otherProfile} className="flex gap-4 mb-2 cursor-pointer">
                      ...
                      <div id="name" className="my-auto font-semibold">
                        {detailThread?.users.name}
                      </div>
                    </div>
                        ...
                        <div>
                          ...
                          <div className="flex gap-4 mb-4 ml-10">
                            ...
                            {item.likes > 0 && item.likes < 2 && (
                              <div id="post-comment-one-like" className="text-grey">
                                {item.likes} Like
                              </div>
                            )}
                            <button id="post-comment-reply-button" onClick={handleReply} value={item.name} className="text-grey cursor-pointer">
                              Reply
                            </button>
                          </div>
                              ...
                              <div id="comment-comment" key={item1.id} className="ml-10 relative">
                                <div>
                                  <div id="comment-comment-identity" onClick={otherProfile} className="flex gap-4 mb-2 cursor-pointer max-w-[85%]">
                                    ...
                                    <div id="comment-comment-name" className="my-auto font-semibold">
                                      {item1.name}
                                    </div>
                                  </div>
                                  <div id="comment-comment-content" className="mb-2 ml-10 max-w-[85%]">
                                    {item1.comment}
                                  </div>
                                  <div className="flex gap-4 mb-4 ml-10">
                                    <div id="comment-comment-time" className="text-grey">
                                      {item1.time}
                                    </div>
                                    {item1.likes > 1 && (
                                      <div id="comment-comment-sum-likes" className="text-grey">
                                        {item1.likes.toLocaleString()} Likes
                                      </div>
                                    )}
                                    {item1.likes > 0 && item1.likes < 2 && (
                                      <div id="comment-comment-one-like" className="text-grey">
                                        {item1.likes} Like
                                      </div>
                                    )}
                                    <button id="comment-comment-reply-button" onClick={handleReply} value={item1.name} className="text-grey cursor-pointer">
                                      Reply
                                    </button>
                                  </div>
                                </div>
                ...
                <div className="lg:fixed bottom-0 right-0 left-[50%] bg-white">
                  ...
                  <form id="comment-reply-box" onSubmit={handleSubmit} className="py-2 px-4 md:p-4 flex justify-between text-xs md:text-base">
                    ...
                    <button type="Submit" className="border-none bg-none md:px-4 text-secondary-orange text-xs md:text-sm xl:text-base">
                      Send
                    </button>
                  </form>
                </div>
